...
class WerewolfTest < Minitest::Test
  ...
  def
   test_when_starting_as_a_human_changing_a_second_time_it_becomes_human_again
    # skip
  ...
  def test_consumes_a_victim
    ...
    victim = Victim.new
    werewolf.change!
    werewolf.eats(victim)
    assert_equal 1, werewolf.stomach.length
  end
  ...
  def test_cannot_consume_victim_if_in_human_form
    ...
    victim = Victim.new
    werewolf.eats(victim)
    assert_equal 0, werewolf.stomach.length
  ...
  def test_a_werewolf_who_has_consumed_a_victim_is_no_longer_hungry
    ...
    werewolf.change!
    werewolf.eats(victim)
    assert_equal @hungry = false, werewolf.hungry?
  end
  ...
  def test_a_werewolf_who_has_consumed_a_victim_makes_the_victim_dead
    ...
    werewolf.change!
    werewolf.eats(victim)
    assert_equal victim.status = :dead, victim.status
  end
